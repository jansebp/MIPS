        ...
        if i == 0:
            ...
            for d in range(0, MAX_BIT_VALUE + 1):
                for aux in range(0, 2):
                    for enable in range(0, constants.RANGES.get('enable')):
                        for aux2 in range(0, constants.CLOCK_PERIOD):

                            q = modules.Flop.flopenr(d, enable, clk, RESET_VALUE, q)

                            if q == 'x':
                                f.write(str(clk) + constants.DELIMITER + str(enable) + constants.DELIMITER
                                        + str(RESET_VALUE) + constants.DELIMITER + format(d, formatter)
                                        + constants.DELIMITER + str('x' * MAX_BIT_VALUE.bit_length()) + '\n')
                            else:
                                f.write(str(clk) + constants.DELIMITER + str(enable) + constants.DELIMITER
                                        + str(RESET_VALUE) + constants.DELIMITER + format(d, formatter)
                                        + constants.DELIMITER + format(q, formatter) + '\n')
                    ...
                    for enable in range(0, constants.RANGES.get('enable')):
                        for reset in range(0, constants.RANGES.get('reset')):
                            for aux2 in range(0, constants.CLOCK_PERIOD):
                                ...
                                if q == 'x':
                                    f.write(str(clk) + constants.DELIMITER + str(enable) + constants.DELIMITER
                                            + str(reset) + constants.DELIMITER + format(d, formatter)
                                            + constants.DELIMITER + str('x' * MAX_BIT_VALUE.bit_length()) + '\n')
                                else:
                                    f.write(str(clk) + constants.DELIMITER + str(enable) + constants.DELIMITER
                                            + str(reset) + constants.DELIMITER + format(d, formatter)
